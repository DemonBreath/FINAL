...
func render(dna_bits: String, mutated: Array[int], frame: int, rows: int = 18, width: int = 22) -> String:
	...
	var bit_count: int = dna_bits.length()
	for y in range(rows):
		var angle: float = float((y + frame) % rows) / float(rows) * TAU
		var offset: int = int(round(sin(angle) * (width / 2.6)))
		var left_col: int = (width / 2) + offset
		var right_col: int = width - left_col
		if right_col <= left_col:
			right_col = left_col + 1
		var left_idx: int = (y * 3 + frame) % bit_count
		var right_idx: int = (y * 3 + frame + 1) % bit_count
		var left_bit: String = _bit_color(dna_bits[left_idx], left_idx, mutated)
		var right_bit: String = _bit_color(dna_bits[right_idx], right_idx, mutated)
		var chars: String = " ".repeat(width + 2)
